...
export const getWidgetData = async (config: WidgetConfig<"pocket-id">) => {
  const [usersRes, oidcClientsRes, auditLogsRes] = await Promise.all([
    ...
    tryCatch(
      fetch(`${config.url}/api/oidc/clients?pagination[limit]=1`, {
        ...
        if (!res.ok) {
          throw new Error(`Failed to fetch Pocket-ID OIDC clients: ${res.statusText}`)
        }
        return res.json() as Promise<PocketIdResponse<PocketIdOidcClient>>
      }),
    ),
    tryCatch(
      fetch(`${config.url}/api/audit-logs?sort[column]=createdAt&sort[direction]=desc`, {
        headers: {
          "X-API-Key": config.apiKey,
        },
      }).then((res) => {
        if (!res.ok) {
          throw new Error(`Failed to fetch Pocket-ID audit logs: ${res.statusText}`)
        }
  ...
  const lastSignIn = auditLogsRes.data.data.find((x) => x.event === "SIGN_IN")?.createdAt
