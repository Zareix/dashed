...
export async function getWidgetData(config: WidgetConfig<"traefik">) {
  ...
  const res = await tryCatch(
    fetch(`${config.url}/api/overview`, { headers }).then((res) => res.json() as Promise<Overview>),
  )
  ...
  return {
    routers: overview.http.routers.total + overview.tcp.routers.total + overview.udp.routers.total,
    services:
      overview.http.services.total + overview.tcp.services.total + overview.udp.services.total,
    middlewares: overview.http.middlewares.total + overview.tcp.middlewares.total,
    alerts,
  }
}
