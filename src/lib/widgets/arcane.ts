    ...
    Promise.all(
      environments.data.map(async ({ id }) =>
        fetch(`${config.url}/api/environments/${id}/projects?limit=999`, {
          ...
          if (!res.ok) {
            throw new Error(`Failed to fetch arcane projects for env ${id}: ${res.statusText}`)
          }
...
export const getWidgetCommands = async (config: WidgetConfig<"arcane">): Promise<CommandList> => {
