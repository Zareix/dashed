      ...
      const [vmsRes, lxcsRes, storageRes] = await Promise.all([
        tryCatch(
          fetch(`${config.url}/api2/json/nodes/${nodeName}/qemu`, {
            ...
            if (!res.ok) {
              throw new Error(`Failed to fetch VMs for ${nodeName}: ${res.statusText}`)
            }
        ...
        tryCatch(
          fetch(`${config.url}/api2/json/nodes/${nodeName}/lxc`, {
            ...
            if (!res.ok) {
              throw new Error(`Failed to fetch LXCs for ${nodeName}: ${res.statusText}`)
            }
        ...
        tryCatch(
          fetch(`${config.url}/api2/json/nodes/${nodeName}/storage`, {
            ...
            if (!res.ok) {
              throw new Error(`Failed to fetch storage for ${nodeName}: ${res.statusText}`)
            }
