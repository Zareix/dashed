...
export const getWidgetData = async (config: WidgetConfig<"radarr">) => {
  const res = await tryCatch(
    Promise.all([
      fetch(
        ...
        if (!res.ok) {
          throw new Error(`Failed to fetch Radarr missing movies: ${res.statusText}`)
        }
  ...
  const allHealth = healthRes.filter((h) => h.type === "error" || h.type === "warning")
...
export const getWidgetCommands = async (config: WidgetConfig<"radarr">): Promise<CommandList> => {
