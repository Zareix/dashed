...
import { AlertTriangle, Container, ExternalLinkIcon, HardDrive, Server } from "lucide-react"
...
export const ProxmoxWidget = ({ config }: Props) => {
  ...
  const stoppedVMs = data.flatMap((node) => node.vms.filter((vm) => vm.status !== "running"))
  const stoppedLXCs = data.flatMap((node) => node.lxcs.filter((lxc) => lxc.status !== "running"))
  const inaccessibleStorage = data.flatMap((node) => node.inaccessibleStorage)
  const hasAlerts =
    stoppedVMs.length > 0 || stoppedLXCs.length > 0 || inaccessibleStorage.length > 0
    ...
    <div className="max-w-100">
            ...
            <div className="flex-1 space-y-1 text-xs">
              {stoppedVMs.length > 0 && (
                <div className="text-yellow-600 dark:text-yellow-400">
                  ...
                  <p>{stoppedVMs.map((vm) => vm.name || `VM-${vm.vmid}`).join(", ")}</p>
                </div>
              )}
              {stoppedLXCs.length > 0 && (
                <div className="text-yellow-600 dark:text-yellow-400">
                  <p className="font-medium">Stopped LXCs:</p>
                  <p>{stoppedLXCs.map((lxc) => lxc.name || `LXC-${lxc.vmid}`).join(", ")}</p>
                </div>
              )}
      ...
      {data.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {data.map((node) => {
            const nodeRunningVms = node.vms.filter((vm) => vm.status === "running")
            const nodeRunningLxcs = node.lxcs.filter((lxc) => lxc.status === "running")
