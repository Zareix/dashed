...
export const ArcaneWidget = ({ config }: Props) => {
  ...
  const environmentsDown = data.environments.filter(
    (environment) => environment.status === "partially running" || environment.status === "stopped",
  )
  const projectsDown = data.projects.filter(
    (project) => project.status === "partially running" || project.status === "stopped",
  )
  ...
  if (environmentsDown.length > 0) {
    alerts.push({
      ...
      items: environmentsDown.map((environment) => `${environment.name} (${environment.status})`),
    })
  }
  if (projectsDown.length > 0) {
    alerts.push({
      ...
      items: projectsDown.map((project) => `${project.name} (${project.status})`),
    })
  }
