...
export const VinceWidget = ({ config }: Props) => {
  ...
  const avgDuration = data.reduce((acc, site) => acc + site.visitDuration, 0) / data.length
  const avgBounceRate = data.reduce((acc, site) => acc + site.bounceRate, 0) / data.length
            ...
            >
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-xs text-muted-foreground">
                  {site.visitors.toLocaleString()} visitors • {site.pageviews.toLocaleString()}
                   pageviews • {site.bounceRate.toFixed(0)}% bounce rate •{" "}
